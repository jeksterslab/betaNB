## ---- test-external-betaNB-diff-beta-nb
lapply(
  X = 1,
  FUN = function(i,
                 text,
    ...
    df <- as.data.frame(df)
    object <- lm(y ~ ., data = df)
    nb <- DiffBetaNB(object = NB(object, R = R, seed = 42))
    ...
    foo <- function(data, i) {
      ...
      x <- lm_process$sigmacap
      p_dif <- dim(lm_process$dif_idx)[2]
      std <- .BetaStarofSigma(
        sigmacap = x,
        q = 1 / sqrt(diag(x)),
        k = lm_process$k
      ...
      for (i in seq_len(p_dif)) {
        diff[i] <- std[
          lm_process$dif_idx[1, i]
        ] - std[
          lm_process$dif_idx[2, i]
        ]
      }
      return(diff)
    }
  ...
  text = "test-external-betaNB-diff-beta-nb",
  tol = 0.01,
  n = 1000L,
  p = 3,
  beta = 0.5,
